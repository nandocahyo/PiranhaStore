...
function Header()
{
    ...
    $this->Cell(30,10,'Piranha Sport');

    $this->Ln(10);
    $this->SetFont('Arial','i',10);
    $this->cell(30,10,'Jl Kopi Sawah No 23');

    $this->cell(80);
    $this->SetFont('Arial','',10);
    $this->cell(30,10,'Malang, '.base64_decode($_GET['uuid']).'');
    $this->Line(10,40,200,40);

    $this->Ln(5);
    $this->SetFont('Arial','i',10);
    $this->cell(30,10,'Telp : 0855-3634-7648');
    $this->Line(10,40,200,40);
    ...
    $this->Ln(5);
    $this->SetFont('Arial','i',10);
    $this->cell(30,10,'No Pembelian : '.base64_decode($_GET['inf']).'');
    $this->Line(10,40,200,40);
}
function LoadData(){
	mysql_connect("localhost","root","");
	mysql_select_db("jersey");
...
$pdf = new PDF();
$pdf->SetTitle('Nota : '.base64_decode($_GET['inf']).'');
$pdf->AliasNbPages();
$header = array('Qty', 'Nama Item','Harga' ,'Total Harga');
$data = $pdf->LoadData();
$pdf->AddPage();
$pdf->Ln(20);
$pdf->BasicTable($header,$data);
$filename=base64_decode($_GET['inf']);
$pdf->Output('','Piranha/'.$filename.'.pdf');
?>
